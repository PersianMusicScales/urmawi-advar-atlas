...
const UI = {
  en: {
    metaTitle:"Al-Urmawī Monochord Studio",
    eyebrow:"Systematist School · Interactive Monochord Studio",
    ...
    tabExplore:"Explore", tabConstruct:"Construct", tabListen:"Listen", tabTrain:"Train & Data",
    ...
    soundLab:"Sound laboratory", webAudio:"Web Audio", openPitch:"Open pitch",
  ...
  fa: {
    ...
    tabExplore:"کاوش", tabConstruct:"ساخت", tabListen:"شنیدن", tabTrain:"تمرین و داده‌ها",
    ...
    soundLab:"آزمایشگاه صدا", webAudio:"صدای مرورگر", openPitch:"بسامد پایه",
  ...
  ar: {
    metaTitle:"مختبر المونوكورد للأرموي",
    eyebrow:"المدرسة النظامية · مختبر مونوكورد تفاعلي",
    ...
    tabExplore:"استكشاف", tabConstruct:"البناء", tabListen:"الاستماع", tabTrain:"التدريب والبيانات",
    ...
    soundLab:"مختبر الصوت", webAudio:"صوت المتصفح", openPitch:"التردد الأساس",
  ...
  tr: {
    ...
    tabExplore:"Keşfet", tabConstruct:"Kur", tabListen:"Dinle", tabTrain:"Alıştırma ve Veri",
    ...
    soundLab:"Ses laboratuvarı", webAudio:"Tarayıcı sesi", openPitch:"Temel frekans",
...
function applyStaticLanguage(){
  ...
  $("fullscreenBtn").title=t("fullscreenTitle");
  ...
  const tabKeys=["tabExplore","tabConstruct","tabListen","tabTrain"];
...
const LESSONS=[
 {kicker:"Foundation",status:"Source concept + modern acoustics",title:"String length and pitch",objective:"Understand the reciprocal relation between sounding length and ideal frequency.",copy:"Al-Urmawī begins from the properties of notes and fret division. In this studio the whole sounding string is normalized to 1. Stopping the string leaves a shorter sounding segment; under the ideal monochord model, frequency changes as the reciprocal of that remaining length.",formula:"f / f₀ = 1 / ℓ",note:0,demo:[0,17]},
...
function runValidationTests(){
 ...
 const passed=tests.filter(x=>x.pass).length;$("validationBadge").textContent=`${passed}/${tests.length}`;$("validationCaption").textContent=et("testsPassed",{n:passed,total:tests.length});const list=$("validationDialogList");list.innerHTML="";tests.forEach(x=>{const li=document.createElement("li");li.className=x.pass?"pass":"fail";li.textContent=`${x.pass?"PASS":"FAIL"} — ${x.name}${x.detail?` (${x.detail})`:""}`;list.appendChild(li)});return {passed,total:tests.length,tests}
}
...
applyStaticLanguage=function(){
 ...
 $("sourceBtn").textContent=et("sources");$("viewModeBtn").textContent=viewMode===0?et("both"):viewMode===1?et("theory"):et("practical");$("instrumentDisclosure").textContent=et("instrumentDisclosure");$("lessonListTitle").textContent=et("guided");$("sourceBasisTitle").textContent=et("evidence");$("openSourcesFromLesson").textContent=et("details");$("audioModeLabel").textContent=et("audioModel");$("audioMode").options[0].textContent=et("pure");$("audioMode").options[1].textContent=et("pluck");$("volumeLabel").textContent=et("volume");$("decayLabel").textContent=et("decay");$("stopAudio").textContent=et("stopAudio");$("speciesTitle").textContent=et("species");$("builderHead").textContent=et("lab");$("modalApplicationsTitle").textContent=et("fromSpecies");$("playSpecies").textContent=et("hear");$("openSourcesFromModes").textContent=et("openSource");$("validationTitle").textContent=et("validation");$("sourceDialogTitle").textContent=et("dialogTitle");$("closeSourceDialog").textContent=et("close");renderLessons();renderSpecies();runValidationTests();
};
...
$("viewModeBtn").addEventListener("click",cycleView);$("contrastBtn").addEventListener("click",()=>document.body.classList.toggle("high-contrast"));$("textSizeBtn").addEventListener("click",()=>document.body.classList.toggle("large-text"));
...
const FULL_I18N={
 en:{
  ...
  lessons:[
   {kicker:"Foundation",status:"Source concept + modern acoustics",title:"String length and pitch",objective:"Understand the reciprocal relation between sounding length and ideal frequency.",copy:"Al-Urmawī begins from the properties of notes and fret division. In this studio the whole sounding string is normalized to 1. Stopping the string leaves a shorter sounding segment; under the ideal monochord model, frequency changes as the reciprocal of that remaining length."},
 ...
 ar:{
  ...
  lessons:[
   {kicker:"الأساس",status:"مفهوم من المصدر + صوتيات حديثة",title:"طول الوتر وارتفاع النغمة",objective:"فهم العلاقة العكسية بين طول الجزء المصوّت والتردد المثالي.",copy:"يبدأ الأرموي من خواص النغم وتقسيم الدساتين. في هذا المختبر يُطبّع طول الوتر المصوّت الكامل إلى ١. وعند إيقاف الوتر يبقى جزء أقصر مصوّت، وفي نموذج المونوكورد المثالي يتغير التردد بحسب مقلوب ذلك الطول."},
...
function applyComprehensiveLanguage(){
 ...
 $("sourceBtn").title=fullText("sourceTitle");$("viewModeBtn").title=fullText("viewTitle");$("contrastBtn").title=fullText("contrastTitle");$("textSizeBtn").title=fullText("textTitle");
...
const PREMIUM_I18N={
 en:{
  ...
  modeLearn:"Guided curriculum",modeExplore:"Pitch explorer",modeConstruct:"Geometrical construction",modeListen:"Sound laboratory",modeModes:"Species & modes",modeTrain:"Training & exact data",
  focusTitle:"Open presentation focus mode",exitFocusTitle:"Return to the full learning studio",fullscreen:"Open in fullscreen"
 },
 fa:{
  ...
  modeLearn:"درس‌های هدایت‌شده",modeExplore:"کاوش جایگاه‌ها",modeConstruct:"ساخت هندسی",modeListen:"آزمایشگاه صدا",modeModes:"اجناس و ادوار",modeTrain:"تمرین و داده‌های دقیق",
  focusTitle:"گشودن نمای متمرکز برای ارائه",exitFocusTitle:"بازگشت به استودیوی کامل",fullscreen:"نمای تمام‌صفحه"
 },
 ar:{
  ...
  modeLearn:"منهج موجّه",modeExplore:"استكشاف المواضع",modeConstruct:"الإنشاء الهندسي",modeListen:"مختبر الصوت",modeModes:"الأجناس والأدوار",modeTrain:"التدريب والبيانات الدقيقة",
  focusTitle:"فتح وضع التركيز للعرض",exitFocusTitle:"العودة إلى الاستوديو الكامل",fullscreen:"ملء الشاشة"
 },
 tr:{
  ...
  modeLearn:"Yönlendirilmiş dersler",modeExplore:"Ses konumu gezgini",modeConstruct:"Geometrik kurulum",modeListen:"Ses laboratuvarı",modeModes:"Cinsler ve devirler",modeTrain:"Alıştırma ve kesin veri",
  focusTitle:"Sunum odak görünümünü aç",exitFocusTitle:"Tam öğrenme stüdyosuna dön",fullscreen:"Tam ekran"
 }
};
...
function applyPremiumLanguage(){
 ...
 const fs=$("fullscreenBtn");if(fs){fs.textContent="⛶";fs.title=premiumText("fullscreen");fs.setAttribute("aria-label",premiumText("fullscreen"))}
