...
const UI = {
  ...
  tr: {
    metaTitle:"el-Urmevî Monokord Atlası",
    eyebrow:"Sistemci Okul · Etkileşimli Monokord Atlası",
...
const LOCALIZED_MISC={
  en:{frequencyFormula:"frequency",score:"Score",correct:"correct"},
  fa:{frequencyFormula:"بسامد",score:"امتیاز",correct:"درست"},
  ar:{frequencyFormula:"التردد",score:"النتيجة",correct:"صحيحة"},
  tr:{frequencyFormula:"frekans",score:"Puan",correct:"doğru"}
};
function miscText(key){return (LOCALIZED_MISC[uiLanguage]||LOCALIZED_MISC.en)[key]||LOCALIZED_MISC.en[key]||key}
...
updateExplore=function(){
  ...
  $("formulaBox").textContent=`${miscText("frequencyFormula")} = ${fixed(base,2)} × (${n.lenD}/${n.lenN}) = ${fixed(base*frequencyRatio(n),2)} Hz`;
...
/* ==========================================================================
   ...
   accessibility and exact-model integrity checks.
   ========================================================================== */
const EXT_UI={
...
runValidationTests=function(){
 ...
 const badge=$("validationBadge"),caption=$("validationCaption"),list=$("validationDialogList");
 if(badge) badge.textContent=`${passed}/${tests.length}`;
 if(caption) caption.textContent=et("testsPassed",{n:passed,total:tests.length});
 if(list){list.innerHTML="";tests.forEach(x=>{const li=document.createElement("li");li.className=x.pass?"pass":"fail";li.textContent=`${x.pass?fullText("pass"):fullText("fail")} — ${x.name}${x.detail?` (${x.detail})`:""}`;list.appendChild(li)})}
